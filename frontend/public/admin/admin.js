...
const AdminPanel = {
  ...
  renderUsageStats() {
    ...
    // Compute actual sum of all resume uploads (treat legacy profiles with a resume as having 1 upload)
    const totalUploads = profiles.reduce((sum, p) => sum + (p.resumeUploadCount || (p.resumeName || p.resumeText ? 1 : 0)), 0);

    const set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
    set('statTotalUploads',     totalUploads);
